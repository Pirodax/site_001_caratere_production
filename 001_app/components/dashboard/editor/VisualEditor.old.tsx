...
function ViewportWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex-1 bg-gray-50 p-4 overflow-y-auto">
      <div
        className="shadow-2xl transition-all duration-300 mx-auto bg-white"
        style={{
          width: state.viewport === 'desktop' ? '100%' : state.viewport === 'tablet' ? '768px' : '375px',
          maxWidth: state.viewport === 'desktop' ? '100%' : state.viewport === 'tablet' ? '768px' : '375px',
        }}
      >
        {children}
      </div>
